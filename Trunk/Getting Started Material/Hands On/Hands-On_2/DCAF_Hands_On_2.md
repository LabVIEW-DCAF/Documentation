...
|*Figure 1.10*
  ...
  2. Drop down the following functions: 1 Unbundle by Name, 2  Bundle by Name function.
  3. Wire the Unbundle by Name function to the Parameters to process cluster. Expand all the terminals.
  4. Wire **Parameters to process.Temperature** to **PID.vi process variable input**.
  5. Wire Parameters to **process.setpoint** to **PID.vi setpoint input**.
  6. Create a constant from **Results from PID.vi output range input**. and wire it to **Bundle by Name**.
  7. Bundle Parameters to **process.output range high** and **Parameters to process.output range low** and wire the cluster to **PID.vi output range input.**
  8. Create a constant from **PID.vi PID gains input**. and wire it to **Bundle by Name**.
  9. Bundle **Parameters to process.P**, **Parameters to process.I**, and P**arameters to process.D** and wire the cluster to **PID.vi PID gains input**.
  10. Create a constant from Results from processing indicator and connect it to the Bundle by Name input cluster terminal. Expand the Unbundle by Name Function to have 2 terminals.
  11. Wire PID.vi output to **Output** from **processing.Output**.
  12. The result should look something like Figure 1.9.
...
|![Figure 1.13 Custom Temperature Controller Includes Block Diagram](Pictures/state_machine_state_diagram.jpg)|
|:--:|
| *Figure 2.1*  |
...
|![Figure 1.13 State Machine UI.](Pictures/state_machine_ui.jpg)|
|:--:|
| *Figure 2.2*  |
